...
| 14g | F2 Failure-State Actionability / Perfect-Information Value Bound | **D — ACTIONABILITY NEGATIVE**（oracle upper bound，非策略；D20 n=12,590/752 days；O1 PERFECT FINAL-LOSER eventday Δ −2.28pp（HAC [−3.04,−1.52] / boot [−3.03,−1.44] 全显著负）、O2 −5.19pp、O3 −4.81pp——即使完美 oracle 提前退出也破坏 BB 均值回归 edge；TP benefit mean −4.13pp（正确退出 loser 反而多亏）、FP cost −24.6~−30.3pp（误杀恢复者）；confusion grid 28/28 cell 全负，TPR=.5/FPR=.2 Δ −1.94（CI [−2.21,−1.66]）；break-even FPR 全负无解（FPR=0 也负）、break-even precision>1；anchor-close 乐观 −1.69pp 仍负；capital 释放 36.8 天/笔真实存在但独立 expectancy 为负） | DEVELOPMENT DIAGNOSTIC（WAITING EXTERNAL AUDIT，未写入 README CURRENT TRUTH） | `[research/risk/FAILURE_STATE_F2.md](research/risk/FAILURE_STATE_F2.md)` |

...
| 14h | F2.1 Matched-Share Actionability / Perfect-Label Fixed-Action Value | **PROVISIONAL B — NARROW POSITIVE ACTIONABILITY（BREAK-EVEN/PRECISION MODULE INVALID，WAITING F2.2）**。核心 matched-share O1 证据**有效**：61,828 natural exit replay parity 0 误差；D20 n=12,590/752 days；O1 完美最终-loser 标签 D20+1 清仓 eventday **+1.45pp**（HAC [0.48,2.42] / boot [0.40,2.61] 显著正）；O2 −0.34pp、O3 −0.09pp（跨 0）；TP −0.46pp、FP −17.9~−25.0pp；future-add 57.0%（F2 分母污染 +4.77pp）。**但 break-even/precision 模块无效**：day_agg 的 pref_d×mean_d 分解混合了 failure/recovery delta，导致 "TPR=.5 break-even FPR=0.080" 与 "TPR=.5/FPR=.2 expected +0.19" 逻辑矛盾；旧 "precision≈96%" 作废，待 F2.2 用 A_d/B_d 正确分解重算。 | DEVELOPMENT DIAGNOSTIC（PROVISIONAL B；break-even 模块待 F2.2，未写入 README CURRENT TRUTH） | `[research/risk/FAILURE_STATE_F21.md](research/risk/FAILURE_STATE_F21.md)` |

...
**R1.4（2026-09-04）：** 正式接受 **S1**（外部审计 PASS）：① **BB threshold = D — HARMFUL**——"等到 entry k=2.5/3.0 才买"显著有害（B25 vs B20_ONLY −2.12pp、B30 vs B25_ONLY −1.91pp，HAC/calendar CI 全 <0；仅 2024 年正）；**禁止**表述为"更深的 contemporaneous BB_Z 有害"（S1 同时发现 B20 首次触发信号中 signal-date 当天更深的 BB_Z bin 有更高 expectancy 迹象——即 depth 的**同日排序价值**与**延迟阈值**是两个不同问题）；② **RSI = C — NO STABLE INCREMENT**，不得进 portfolio；③ **Sector = N/A — PIT DATA NOT READY**，不得用当前行业分类回填。**关闭 HIGHER ENTRY K branch**（禁 2.1/2.2/… sigma scan、2.5/3.0 portfolio gate、RSI threshold scan、RSI portfolio test）。开启 **CONTEMPORANEOUS BB DEPTH RANKING（S1.1）**：不改变 B20 入场条件（仍为 close_adj < MA20−2·SD20，不等待 -2.5σ/-3σ），只回答"同一个 signal day 多个 B20 候选时，当天立即可见的 BB_Z 深度能否用于排序（DEEP30/MID40/SHALLOW30），把有限 K slots 优先给更有价值的信号"。THRESHOLD GATING（已否定）与 CONTEMPORANEOUS RANKING（本轮研究）严格区分；不得延迟入场。2025–2026 CLOSED。

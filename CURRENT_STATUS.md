...
| 14r | W1 Multi-Timeframe Bollinger Diagnostic（日线 B20 × real-time weekly BB） | **D — HARMFUL（外审 ACCEPTED）**：B20 parity 63,785 ✓；weekly warmup 覆盖 98.60%（按年 97.99–99.56%）；W_LOW_TOUCH=1 n=5,547（8.82%）；同日配对 delta（touch−no_touch）= **−0.686pp**，HAC [−1.421,+0.049]、calendar [−1.442,+0.039]（跨 0）；同日 LOW30−HIGH30 = −0.555pp（HAC/calendar 两 CI 全负）；between-day 低周线日 +2.57pp（日期效应强）；TOUCH 日全A收益 −2.00% vs −1.47%（集中在普跌日）；MAE≤−30 14.0% vs 9.8%（+4.1pp 恶化）；pooled Spearman −0.117 / day-level +0.028（横截面反向）；逐年正方向仅 1/5；**W2 DO NOT START** | **ACCEPTED AS D**；weekly filter/trigger/exit/参数扫描全部关闭；措辞：W1 pooled touch 优势主要表现为 **BETWEEN-DAY composition effect**（与弱全市场交易日高度相关），不得写成已严格证明 100% 由市场导致 | `[research/multitimeframe/WEEKLY_BB_W1.md](research/multitimeframe/WEEKLY_BB_W1.md)` |
| 14s | B1 B20 Signal Breadth / Crowding Diagnostic（日级信号广度） | **A — STRONG BREADTH VALUE（B1.1 修正后 FINAL，外审 ACCEPTED）**：B20 parity 63,785；signal days 1,110 / zero-signal days 102；Q1→Q5 DAY_MEAN_RETURN 2.26→4.92%（单调）；Spearman +0.160；**Q5−Q1 = +2.664291pp**（correct calendar bootstrap **CI [+1.102,+4.185]**、mean +2.674）；rank-slope HAC（matrix）**[+0.00130,+0.00463]**；条件回归 b1=+3.283183（**正确 HAC CI [+1.030,+5.537]**，控制当日全A收益后仍显著）；**5/5 年正**、逐年 Q5−Q1 全正；tail 不恶化（Q5 MAE30 9.1% < Q1 12.5%）；capture 语义：FULL_MARKET_SIGNAL_CAPTURE_RATIO Q5=0.037%，但 **P5 actual pipeline 在 Q5 日仅 201 候选（amount Top10 截断 −99.6%），其中 K 挡 65.2%** → 两级漏斗（amount 截断为主 + K 容量为次），非纯 K 责任。**B1 旧 bootstrap CI [+0.335,+2.307] 与旧条件 CI [+3.267,+3.299] 正式撤销**（signed combined mean / b2 var_k 复用 bug），以 B1.1 corrected inference 为唯一有效版本 | **ACCEPTED**（B1.1 外审通过；B FINAL=A；P7 允许） | `[research/breadth/B20_BREADTH_B11.md](research/breadth/B20_BREADTH_B11.md)` |
| 14t | B1.1 Breadth Statistical Inference + Capture-Semantics Remediation | **PASS / ACCEPTED（FINAL A — STRONG BREADTH VALUE）**：①Q5−Q1 bootstrap 修正为每 replicate 内 mean(Q5)−mean(Q1)（toy Q5[5,6]/Q1[1,2]=4.0 ✓）；正确 CI **[+1.102,+4.185]**；②multivariate HAC 用标准 sandwich (X'X)⁻¹S(X'X)⁻¹，statsmodels 逐位 parity ✓；b1 CI **[+1.030,+5.537]**；③univariate rank-slope matrix HAC **[+0.00130,+0.00463]**；④yearly/monotonicity/tail parity 全 PASS；⑤capture 拆为 FULL_MARKET_SIGNAL_CAPTURE_RATIO（改名）与 ACTUAL_PIPELINE_CAPTURE（P5 ledger：Q1→Q5 pipeline 20/75/95/136/201；K_block_rate 50.0/66.7/63.2/62.5/65.2%；admission_rate Q5 8.5% 最低）；I1–I10 全 PASS | **ACCEPTED**（B1.1 外审通过；P7 ALLOWED；P7 设计须针对两级漏斗机制） | `[research/breadth/B20_BREADTH_B11.md](research/breadth/B20_BREADTH_B11.md)` |
...
| 15s | P7 Panic-Breadth Capacity Architecture | **D — HARMFUL（A2 mechanism M4 — BREADTH ALPHA NOT PORTFOLIO-CONVERTIBLE VIA CAPACITY EXPANSION；外审 ACCEPTED）**：530-vs-527 bridge **exact closed**（3 缺失 = 2 replay-held exclusion + 1 end-horizon）；A0 parity PASS（+30.295094%/76/−30.78973%/0.346765）；PANIC80（expanding 80th pct、252 前交易日）188/1,110 信号日；**A1（panic K6）= −20.81% / MaxDD −46.94% / Sharpe −0.047 / PnL −208,123**，A2（panic Top20+K6）= −22.79% / −44.57% / −0.094；A1−A0 = **−511,074 PnL**；逐年 **0/5 年改善**；path bridge：COMMON 51 笔 **delta −183,557**（共享资本/槽位稀释）、A0_ONLY 25 笔 **+415,753**（被 panic 新仓挤掉的 A0 优质交易，独立 +7.81%/win 83.3%/MAE30 0%）、A1_ONLY 32 笔 +88,236（独立 +5.77% 但补不上）；blocked_K 336→236、**exec_no_cash 0→77**（1M 资本 K6 下耗尽）；A2_ONLY MAE30 20%（Top20 候选更差）；增量集中度 Top1 date 13.9%（系统性恶化非单日）；**K=3 继续保留为 protective admission constraint（第 4 次一致证据）** | **ACCEPTED**（P7 外审通过；panic capacity expansion branch CLOSED） | `[research/portfolio/PANIC_CAPACITY_P7.md](research/portfolio/PANIC_CAPACITY_P7.md)` |
...
**R1.5+D1+D1.1（2026-09-04）：** 正式接受 **S1.1 = C — NO STABLE RANKING VALUE**（外部审计 PASS）。同日 BB_Z 相对排序无稳定横截面价值；collision 场景方向为负；TOP3 深度 vs 成交额无差异；**不进入 K=3 portfolio test**。关闭 **CONTEMPORANEOUS BB DEPTH RANKING branch**；REPEAT_HIT exploratory finding **NOT REGISTERED FOR DEVELOPMENT**。开启并完成 **PIT CONTEXT DATA FOUNDATION（D1）**：只建立 2020–2024 每个 B20 signal date 可审计的 PIT SECTOR 与 PIT FUNDAMENTAL 数据层（ann_date <= signal_date 硬规则、revision as-of selector、TTM 只用已披露季度）；**DATA FOUNDATION ONLY**——禁止交易策略测试、禁止读取 outcome 后挑财务指标、禁止打开 2025–2026。SECTOR 达 A/B 才允许 S2（sector strength）；FUNDAMENTAL 达 A/B 才允许 S3（fundamental distress）；若两者都 A/B 优先 S3（技术性错杀 vs 永久性基本面重估假设，本阶段不测试）。**D1.1（2026-09-04）**：外部审计指出 D1 revision tie-break 与 Registry 潜在不一致 → D1 **HOLD**；D1.1 完整实现 STRICT_SELECTOR（ann_date≤T→max ann→max update_flag→max f_ann_date→row-hash）并给 fina_indicator 同日重复建立 **AMBIGUOUS→NA** 规则；结果：版本标识 OLD vs STRICT **0 差异**，但同公告日多版本行的数值在 ~12% 信号上被修正（revenue_ttm 8,064 / netprofit_ttm 8,348 / ocf_ttm 7,793 changed），fina **1,179 事件（1.85%）置 NA**；1,179 全查 0 fail；TTM 全量 future_component_count=0；coverage 不变；**D1.1 PASS=True**（sector B / fundamental A）。S3 的启动条件 = 外部审计确认 D1.1。**D1.2（2026-09-04）**：外审再发现 Registry 更根本的 visibility 问题（ann_date=公告日期 vs f_ann_date=实际公告日期）；审计结论：**f_ann_date 字段混合两种语义**——小 delta（1–30d）是真实实际披露日（巨潮 11 例中 8/11 精确匹配，晚间公告场景），大 delta（>180d）是**库更新时间戳**（同一股票全报告期同一天；巨潮 30 例 0/30 匹配）；信号级 FUTURE_ACTUAL 组件 3,466 个中 **91.5% 为大 delta 假警报**，真泄漏候选仅 **14 个 cur 组件（≤0.02%）至 249 个（≤0.39%）**；RULE_B 会对 381 信号（0.60%）造成错误修正；**D1.2 分类 = C SEMANTICS AMBIGUOUS**（外审 ACCEPTED），D1 维持 HOLD 不 ACCEPT；S3 仍禁止（需更可靠公告日源或治理登记混合规则，禁止事后发明混合规则）。**R1.6（2026-09-04）**：接受 D1.2=C，fundamental branch PAUSED；开启 **W1 MULTI-TIMEFRAME BOLLINGER DIAGNOSTIC**（日线 B20 × real-time weekly BB state；纯诊断，不改 entry/exit，2025–2026 CLOSED）。**W1（2026-09-04）**：同日 paired touch−no_touch=−0.686pp（HAC/calendar 跨 0），同日 LOW30−HIGH30 两 CI 全负，tail MAE≤−30 恶化 +4.1pp，逐年正方向 1/5 → **D — HARMFUL**；pooled touch 优势 = between-day composition effect（普跌日集中），非横截面价值。**R1.7（2026-09-04）**：接受 W1=D，关闭 multi-timeframe strategy branch；开启 **B1 — B20 SIGNAL BREADTH / CROWDING DIAGNOSTIC**（date-level：当日 B20 信号数量/广度 vs 当日独立 episode expectancy；纯诊断，K=3 不变，2025–2026 CLOSED）。**B1（2026-09-04）**：signal days 1,110（B20 parity 63,785）；Q1→Q5 DAY_MEAN_RETURN 2.26→4.92% 单调；Spearman +0.160；Q5−Q1 +2.664291pp；**B1.1 修正（外审 HOLD 后）**：Q5−Q1 bootstrap 改为每 replicate mean(Q5)−mean(Q1)（toy=4.0 ✓），正确 CI **[+1.102,+4.185]**；multivariate HAC 用标准 sandwich，b1 CI **[+1.030,+5.537]**（statsmodels 逐位 parity）；univariate rank-slope matrix HAC **[+0.00130,+0.00463]**；yearly/monotonicity/tail parity 全 PASS；capture 拆为 FULL_MARKET_SIGNAL_CAPTURE_RATIO（改名，Q5=0.037%）与 ACTUAL_PIPELINE_CAPTURE（P5 ledger：Q1→Q5 pipeline 20/75/95/136/201、K_block_rate 50.0/66.7/63.2/62.5/65.2%、admission_rate Q5 8.5% 最低）→ 两级漏斗（amount Top10 截断为主 + K 容量为次），"17/46,310"不构成"全被 K 挡"的证据。**B1 FINAL = A — STRONG BREADTH VALUE（外审 ACCEPTED）**；旧 bootstrap/条件 CI 正式撤销；P7 允许且设计须针对两级漏斗。**R1.8（2026-09-04）**：正式接受 B1.1=A（STRONG BREADTH VALUE）；正式撤销 B1 旧 bootstrap CI [+0.335,+2.307] 与旧条件 CI [+3.267,+3.299]，以 B1.1 corrected inference 为唯一有效版本；开启 **P7 PANIC-BREADTH CAPACITY ARCHITECTURE**（只测 A0/A1/A2 三冻结结构；expanding PANIC80、252 前交易日、T 信号只影响 T+1 准入、K6→K3 回落不强卖；530-vs-527 bridge 前置）。**P7（2026-09-04）**：530-vs-527 bridge **exact closed**（3 缺失 = 2022-05-13 002415.SZ replay-held exclusion + 2023-11-09 601318.SH replay-held exclusion（replay-vs-portfolio 持仓差异）+ 2024-12-31 600030.SH end-horizon；非数据/定义错误）；PANIC80=188/1,110 信号日（2020 全年 0，252 日历史 2021 年初才满足）；A0 parity 5 项 PASS；**A1（panic K6）−20.81% / MaxDD −46.94% / Sharpe −0.047**，A2（panic Top20+K6）−22.79% / −44.57% / −0.094，逐年 **0/5 年改善**；path bridge：COMMON 51 笔 delta **−183,557**（共享资本/槽位稀释）、A0_ONLY 25 笔 **+415,753**（独立 +7.81%/win 83.3%/MAE30 0%——panic 新仓挤掉了 A0 更优质交易）、A1_ONLY 32 笔 +88,236（独立 +5.77% 但补不上）；exec_no_cash 0→77（1M 资本 K6 下耗尽）；A2_ONLY MAE30 20%；增量集中度 Top1 date 13.9%（系统性恶化非单日）→ **D — HARMFUL / M4 — BREADTH ALPHA NOT PORTFOLIO-CONVERTIBLE VIA CAPACITY EXPANSION**；K=3 第 4 次一致证明为 protective admission constraint（P4/P4.1/P6/P7）。

**R1.9（2026-09-04）：** 正式接受 **P7 = D — HARMFUL（机制 M4）** **M1（2026-09-04，EXPLORATORY PROTOCOL-DEVIATED / WAITING RECONCILIATION）**：M1 指令于 D 节（PANIC80 处）截断，Agent 按可见意图补全规格并结果前 prereg（`661e81f`/`bee0efa`，`research/market_rebound/`）；外审判定存在 MATERIAL DEVIATIONS：primary 应为 FWD5（实际 FWD20）、horizons 应为 1/3/5/10/20（实际 5/10/20/40）、continuous 应为 EXPANDING_BREADTH_RANK01（实际 raw Spearman+full-sample Q1-Q5）、conditional 应为 expanding-rank（实际 panic dummy）、yearly 分母应为 2021-2024（实际写 2020-2024）、缺 cluster-first robustness、缺 5d tail/drawdown、benchmark hierarchy 未完整执行。**M1=B 与 ETF carrier YES 正式 WITHDRAWN AS FINAL / 不写入 CURRENT TRUTH**；ETF gate=HOLD；当前结果仅作 descriptive evidence（H5 +0.123pp / H10 +0.475pp / H20 +0.430pp / H40 +0.733pp，CI 全跨 0；Q5−Q1 连续关系为负）。M1.1 恢复外部冻结协议（PRIMARY FWD5、EXPANDING_BREADTH_RANK01、2021-2024 分母、cluster-first、5d tail、PIT equal-weight fallback）。 **M1.1（2026-09-04，PROVISIONAL / PENDING M1.2）**：恢复外部协议后 FWD5 delta +0.275pp（panic +0.326% vs normal +0.051%；3/4 年正；HAC [−0.300,+0.851]、bootstrap [−0.277,+0.787] 均跨 0；continuous slope −0.319 为负；cluster-first 96 cluster 反转 −0.369pp；DD5 panic 略深 0.30pp 但无极端尾部恶化）。**B_NARROW_MARKET_TRANSLATION 标记 PROVISIONAL**：外审发现 3 处实现偏差（bootstrap 在 signal-day 序列而非 full 1212-day calendar 上抽块；cluster 按 signal-row 相邻而非 trading-day index 相邻；DD5 normal comparator 混入 2020 warmup），**ETF gate HOLD** 直至 M1.2 remediation（只修 3 项，其他定义全部不变）。；**PANIC-DAY CAPACITY EXPANSION branch CLOSED**；禁止继续 K4/K5/K8、Top15/Top30、PANIC70/75/85/90 等参数搜索。措辞精确化：现有 development evidence 反复显示**解除/扩张 K 会恶化共享资本路径**（非证明 K=3 理论最优）；K=3 继续保留为 protective admission constraint。B1.1=A（signal-level breadth alpha）仍成立。开启 **M1 — PANIC-BREADTH → MARKET REBOUND TRANSLATION**：纯 market-level diagnostic，不改变股票组合，只回答"当很多股票同时跌破 B20 时，后面的 A 股整体指数是否更容易反弹"（YES 才允许未来考虑 broad-market ETF/basket carrier；NO 则不做 ETF architecture）。2025–2026 CLOSED。

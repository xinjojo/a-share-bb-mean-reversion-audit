...
- **SIGPATH（2026-09-05，全量信号路径数据层，只建事实层）：** 用户协议要求把 S1 frozen B20 入场信号出现后所有符合条件事件全量展开（不抽样、不删观测），每笔可人工回到 K 线，禁止输出策略结论。**prereg** `SIGNAL_PATH_AUDIT_REGISTRY.csv`（commit `94bd041` SIGPATH-A → A2 `483e72b7` → **A3 `ea36927`**，SHA `4336000d…`，结果前冻结；universe 修正：**NEW_ENTRY = 63,887 = 63,785 已平仓（TP 61,828+FS 1,957）+ 102 期末 2024-12-31 无行情未结算，均为真实成功入场**；layers = Σlevels over 63,887 episodes）。**冻结定义**：BB=rolling(20) mean/std ddof=1、close_adj<MA20−2SD20 且当日非跌停、eligible=listed≥60d+PIT ST+BB20 有值、entry=T+1 open、entry_cost=open×1.001（10bp 滑点）、100 股 lot、200k/层、MAX_LEVELS=5；ret=price/entry_cost−1（不截断负值）；D1=entry_date（该股实际交易日，停牌不计 horizon）；MFE_Dn=max(high_ret D1..Dn)、MAE_Dn=min(low_ret D1..Dn)；signal_id=SIG{episode_id}_{layer_idx}；data_quality_flag 只标记不删除（JUMP=相邻 close 跳变≥30%、SHORT_HISTORY=available_future_days<20）；**N 限定 2024-12-31，2025+ 零读取**。**结果（commit `bc7e989` SIGPATH-B）**：**157,469 条独立 signal**（NEW_ENTRY 63,887 / ADD_ON_1 42,105 / _2 25,652 / _3 15,828 / _4 9,997），覆盖 **5,166 只股票**，signal_date 2020-02-06 ~ 2024-12-30（entry 至 2024-12-31）；**wide 157,469×251**（每 signal 1 行、D1–D20 全 OHLC + 4 ret + MFE/MAE）+ **long 3,149,380×22**（signal×day 固定 20 行，43,031 缺失日 NaN 行保留）；Parquet + CSV 分片（wide 4 片 / long 8 片）。**描述统计（独立回读验证）**：D20 close_ret n=154,333 mean **+1.486%** / median **+0.247%** / std 13.39% / P10 **−12.02%** / P90 **+15.99%** / min **−90.28%** / max **+375.01%**（极端保留）；D1 close median −0.044%、D5 +0.210%、D10 +0.129%；D10 MFE median +4.84% / MAE median −4.73%。**质量**：SHORT_HISTORY 3,135（期末截断保留）、JUMP 341+1（标记不删）；**stock_name = PIT 名称**（namechange_full 按 signal_date 当时名称；退市股可追溯，300116→坚瑞沃能、300156→神雾环保）；industry_snapshot/list_date 当前快照（非 PIT，仅人工定位）。**sanity**：随机 20 + 大涨 5 + 大跌 5 + 先跌后涨 5 + 先涨后跌 5，ret/MFE/MAE 公式机器断言 PASS（全天高于/低于成本、日内跨越成本三态验证）。**交付**：manual_review_index.csv（157,469 行）、signal_path_descriptive_statistics.csv、signal_path_distribution_bins.csv（18 桶）、mfe/mae_hit_rate_matrix.csv（D1–D20 × ±1..±20%）、charts/ 11 张（fig1–3 histogram、fig4–6 percentile path、fig7 三张 heatmap、fig10–11 hit-rate heatmap）、README.md。**本阶段无 A/B/C/D 结论分类、不判断策略有效性、未改任何参数、未跑组合、未读 2025–2026**。canonical: `research/sigpath/README.md` + `results/evidence/sigpath/`。
...
| SIGPATH Signal Path Audit Registry | `research/sigpath/registries/` | `94bd041`（SIGPATH-A）→ `483e72b7`（A2）→ `ea36927`（A3，SHA `4336000d…`） |

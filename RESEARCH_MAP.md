...
S1 Signal Selectivity ──► BB threshold D HARMFUL / RSI C / Sector N/A (ACCEPTED)
   │
   ▼
★ CURRENT: D1 PIT Context Data Foundation（PIT 行业归属 + 财务披露 as-of 数据层，DATA FOUNDATION ONLY）★
```
...
- **D1（进行中，DEVELOPMENT DIAGNOSTIC）：** PIT CONTEXT DATA FOUNDATION — 为 2020–2024 每个 B20 signal date 建立可审计 PIT SECTOR（历史行业归属，current snapshot 禁回填）与 PIT FUNDAMENTAL（ann_date<=signal_date 硬规则、revision as-of selector、TTM 只用已披露季度）数据层；coverage/spot-check/red-team；**DATA FOUNDATION ONLY**，禁止策略测试与 outcome 读取；SECTOR A/B → S2，FUNDAMENTAL A/B → S3（都 A/B 优先 S3）；2025–2026 CLOSED。registry: `research/context/registries/PIT_CONTEXT_D1_REGISTRY.csv`。

...
### 15. Project Alpha Inventory / Blind-Test Decision Gate（A0，已收官 DECISION B）
...
- **M1.1（2026-09-04，PROVISIONAL→M1.2 修正后 ACCEPTED）：** EXTERNAL-PROTOCOL RECONCILIATION（M1 因提示词截断导致 protocol deviations，正式降为 EXPLORATORY；M1=B 与 ETF YES withdrawn）。恢复外部冻结协议：PRIMARY FWD5、horizons 1/3/5/10/20、continuous=EXPANDING_BREADTH_RANK01、conditional=expanding rank、year 分母 2021–2024、cluster-first、5d tail/DD5、market series hierarchy（官方宽基→PIT equal-weight fallback；仓库无 000985 全市场宽基 → **PIT_EQUAL_WEIGHT_FALLBACK**，与 B1 universe_size 逐日 parity 1212 天 ✓）。结果：**FWD5 delta +0.2752pp**（panic +0.326% vs normal +0.051%，HAC [−0.300,+0.851] / boot [−0.277,+0.787] 跨 0）；continuous slope −0.319（负）；conditional b1 −0.213（负）；**3/4 年正**；cluster-first **−0.369pp（反转）**；tail 无极端恶化；分类 **B_NARROW_MARKET_TRANSLATION（PROVISIONAL）**。canonical: `research/market_state/PANIC_BREADTH_MARKET_M11.md` + `results/evidence/m11/`。Registry SHA `bf10c749…`（commit `852635a`）。
- **M1.2（2026-09-04，ACCEPTED）：** CALENDAR/CLUSTER/TAIL REMEDIATION（只修 3 项实现偏差：bootstrap 沿完整 1,212 交易日历抽块、cluster 按 trading-day index 相邻、DD5 与 primary 同样本）。corrected full-calendar bootstrap CI **[−0.288,+0.798]**（点值保持 +0.2752pp，结论不变）；cluster-first **−0.2766pp**（96=96 clusters 0 拆分）；DD5 panic −1.715% vs normal −1.448%（−0.267pp，无 2020 混入、无极端尾部恶化）；**FINAL B — NARROW MARKET TRANSLATION（外审 ACCEPTED）**；**ETF gate = YES — WORTHY OF ONE FROZEN CARRIER TEST**（不是 validated）。措辞：PANIC80 has a NARROW, STATISTICALLY WEAK, EPISODE-DEPENDENT market-rebound indication（cluster-first 负 → 正效应依赖重复 panic 日期）。canonical: `research/market_state/PANIC_BREADTH_MARKET_M12.md` + `results/evidence/m12/`。Registry SHA `524ae9e4…`（commit `edef30a`）。
- **M2（2026-09-04，DEVELOPMENT RESULT / D — HARMFUL，待外审）：** ONE FROZEN BROAD-MARKET ETF CARRIER TEST。carrier 由 outcome-free metadata audit 先于 signal-conditioned 回测选定 **510300.SH**（沪深300；2012-05-28 上市；2020–2024 1212/1212 覆盖 0 缺失；median amount ≈229.9e4 万最高）。PANIC80 188 parity ✓；T+1 open 买入、持有恰 5 交易日（exit=close(T+5)）、**单一活跃仓位（持仓期新信号忽略，非重叠）**、100k 独立账户、100 份 lot、佣金 0.025% 单边（min 5 元）+滑点 10bp、无印花税。结果：**78 笔 / net total −5.309% / PnL −5,309 / CAGR −1.13% / MaxDD −23.25% / Sharpe −0.059 / time-in-market 32.18%**；逐年 **2/4 正**（2021 +8.78% / 2022 −3.47% / 2023 **−17.35%** / 2024 +6.72%）；trade bootstrap CI **[−0.607,+0.457]**；permutation **p=0.159**（不显著）；**matched non-panic delta +0.5144pp**（panic 5d 相对普通日略好、方向与 M1.2 一致，但绝对净亏且不显著）；benchmark BUY&HOLD −24.02%（相对少亏 18.7pp，绝对负）；cluster day1 67/78（85.9%）；单笔最大盈利 +6,828 元（占总 PnL −128.6% 因总量负）→ **net negative → D — HARMFUL**：M1.2=B 的 market-rebound indication 无法通过单宽基 ETF carrier 转成 after-cost 经济优势。canonical: `research/etf/PANIC_ETF_CARRIER_M2.md` + `results/evidence/m2/`。Registry SHA `7ff3333e…`（commit `b32dfed` M2-A）。**结果已交外部审计**。
- **M2.1（2026-09-04，DEVELOPMENT RESULT / hygiene remediation，待外审）：** BOOKKEEPING/NULL HYGIENE（M2 substantive D 已定，不改结论）。①commission-aware sizing：旧规则账户现金最大赤字 **−13,206.41 元**（2023 亏损期固定 100k 预算的万元级隐性杠杆），修正后 min cash +2.42、`cash≥−1e-8` 全程 PASS、qty 变化 76/78；②permutation 同年无放回：p=0.145（旧 0.159，仍不显著）；③matched estimand：单次 draw +0.5144pp 撤回，正式 = obs − null_mean = **+0.3257pp**；equity parity diff 0.00 元（tol 0.01）；**gross account +12.05% / net −7.2514%**（滑点 15.44pp+佣金 3.86pp 吃光毛收益；成本拖累是净负机制）；MaxDD −24.13%、PF 0.930、正年份 2/4 → **仍 D — HARMFUL**。canonical: `research/etf/PANIC_ETF_CARRIER_M21.md` + `results/evidence/m21/`。**结果已交外部审计**。
- **A0（2026-09-04，PROJECT ALPHA INVENTORY / BLIND-TEST DECISION GATE）：** 外审 **M2.1 PASS / M2 FINAL = D**；ETF branch CLOSED。停止新研究，盘点全部 development evidence（4 类归档）。**robust signal-level alpha = 2 项**（Independent replay BB 下轨 episode edge；B1.1 date-level breadth edge，Q5−Q1 +2.664pp / boot CI [+1.102,+4.185] / 条件 b1 +3.283 CI [+1.030,+5.537] / 5-5 年正）；**failed architectures** = first-gen（+354.9% INVALID same-bar lookahead）、K999、ML1、fixed stops（S0 确认）、wallet separation（P6 D）、deferred queue（P5.1 C）、panic K6 / Top20+K6（P7 D/M4）、ETF carrier（M2 D）。**当前最好且真实审计的组合 = STRICT_C / A0 K3 baseline（development-only baseline，NOT proven alpha carrier）**：+30.2951% / Trades 76 / MaxDD −30.79% / Sharpe 0.347，after-cost（A 股佣金+滑点）真实执行（T+1、lot、PIT 可交易池）。**Blind-test gate（10 项）：6/10 完整满足 → DECISION B — DO NOT OPEN BLIND TEST**（2025–2026 保持 CLOSED）。核心原因：A0 是 development 反复比较后保留的 baseline，非预注册冻结的部署架构；Sharpe/MaxDD 风险偏弱；同一 development 期 4 次 signal→portfolio 转换（P3/P6/P7/M2）全部失败，转换机制未建立；D1.2 fundamental 语义 AMBIGUOUS 使 context 层未完全冻结。科学结论：**有 signal alpha，无 deployable portfolio alpha**（可接受的科研终点）。canonical: `research/governance/PROJECT_ALPHA_INVENTORY_A0.md` + `results/evidence/a0/`。

### 16. ★ CURRENT: Squeeze/ADX Regime Attribution Audit（REG1，候选 Alpha / Regime Filter 审计）
- **REG1（2026-09-05，post-A0 新审计）：** 用户协议审计 Squeeze / DMI / ADX 能否识别"不应做 BB MR"的状态（**不是修改主策略，不是开发突破策略，不打开 2025–2026**）。prereg `SQUEEZE_ADX_REGIME_REGISTRY.csv`（commit `6b7dfb5` REG1-A，SHA `ce8460a6…`，结果前冻结全部参数与分级门限）。**baseline = S1 frozen B20 independent replay dev n=61,828**（F2.1 parity 0 误差，不重新定义策略）；REGIME_ASOF = signal_date close（T 收盘，入场 T+1 open 前最后可见快照）；价格 = warmup 2018–2019 + combined 2020–2024（≤2024-12-31，2025+ 未读）。**冻结指标**：DMI14/ADX14（Wilder）、BB20/2/ddof1（策略原生）、KC EMA20±1.5·ATR20、LazyBear MOM20、120d BB-width percentile、SQUEEZE_ON/RELEASE/0–3d recent；5 个预注册组 G1–G5；4 个独立 VETO（禁止组合）；指标实现经独立参考实现 4 股全 signal-date 相对误差 0 验证。**结果**：G1 n=27,955（+4.788%）/ G2 n=34,442（+5.325%，bootstrap CI [+0.377,+0.703] **显著更好**，Hedges g 0.05）/ G3 n=14,884（+4.945%，CI 跨 0）/ G4 n=28,090（+5.232%，CI [+0.278,+0.614] 显著更好，g 0.04）/ G5 n=7,015（+4.986%，CI 跨 0）；**tail worst5/10/20% 富集 0.96–1.03×（大亏损无任何富集）**；**VETO 全部有害**：PnL −62.1%/−22.7%/−49.5%/−11.6%，removed 77.6–78.7% 为盈利交易、foregone/avoided 169–184% → **FINAL = HARMFUL**。结论：**bearish trend expansion / squeeze release 不是 BB MR 亏损的聚集地；Squeeze/DMI/ADX 无资格进入下一阶段过滤器研究**；Squeeze/ADX regime-filter branch **CLOSED**（待外审确认升 ACCEPTED）；主策略 STRICT_C/A0 K3 冻结不变；A0 DECISION B 不受影响；2025–2026 UNTOUCHED。Future hypothesis（只记录不开发）：G2/G4 微弱正差可能与 B1.1 date-level breadth edge 重叠。canonical: `research/regime/squeeze_adx_audit_report.md` + `results/evidence/reg1/`（attribution/group_stats/tail_attribution/inference/veto_results/summary/invariants）。

### 17. ★ CURRENT: SIGPATH — A股 BB Mean Reversion 全量 Signal Forward Path Audit（2026-09-05，post-REG1 数据层）
- **SIGPATH（2026-09-05，全量信号路径数据层，只建事实层）：** 用户协议要求把 S1 frozen B20 入场信号出现后所有符合条件事件全量展开（不抽样、不删观测），每笔可人工回到 K 线，禁止输出策略结论。**prereg** `SIGNAL_PATH_AUDIT_REGISTRY.csv`（SIGPATH-A `94bd041d21a695557df3d9596f26385b4fc8f1bf` → A2 `8bb98867988bf2b8d3d0304974391ec9276a72ba` → **A3 `ea369277546d73957f6b5fd94700a03f21788173`**，SHA `4336000d…`，结果前冻结；universe 修正：**NEW_ENTRY = 63,887 = 63,785 已平仓（TP 61,828+FS 1,957）+ 102 期末 2024-12-31 无行情未结算，均为真实成功入场**；layers = Σlevels over 63,887 episodes）。**冻结定义**：BB=rolling(20) mean/std ddof=1、close_adj<MA20−2SD20 且当日非跌停、eligible=listed≥60d+PIT ST+BB20 有值、entry=T+1 open、entry_cost=open×1.001（10bp 滑点）、100 股 lot、200k/层、MAX_LEVELS=5；ret=price/entry_cost−1（不截断负值）；D1=entry_date（该股实际交易日，停牌不计 horizon）；MFE_Dn=max(high_ret D1..Dn)、MAE_Dn=min(low_ret D1..Dn)；signal_id=SIG{episode_id}_{layer_idx}；data_quality_flag 只标记不删除（JUMP=相邻 close 跳变≥30%、SHORT_HISTORY=available_future_days<20）；**N 限定 2024-12-31，2025+ 零读取**。**结果（canonical commit `974fa2aef6fd9284ce50163d0d68a4fb13d3697e` SIGPATH-B）**：**157,469 条独立 signal**（NEW_ENTRY 63,887 / ADD_ON_1 42,105 / _2 25,652 / _3 15,828 / _4 9,997），覆盖 **5,166 只股票**，signal_date 2020-02-06 ~ 2024-12-30（entry 至 2024-12-31）；**wide 157,469×251**（每 signal 1 行、D1–D20 全 OHLC + 4 ret + MFE/MAE）+ **long 3,149,380×22**（signal×day 固定 20 行，43,031 缺失日 NaN 行保留）；Parquet + CSV 分片（wide 4 片 / long 8 片）。**描述统计（独立回读验证）**：D20 close_ret n=154,333 mean **+1.486%** / median **+0.247%** / std 13.39% / P10 **−12.02%** / P90 **+15.99%** / min **−90.28%** / max **+375.01%**（极端保留）；D1 close median −0.044%、D5 +0.210%、D10 +0.129%；D10 MFE median +4.84% / MAE median −4.73%。**质量**：SHORT_HISTORY 3,135（期末截断保留）、JUMP 341+1（标记不删）；stock_name 经 SIGPATH-C 核查：namechange_full 有效区间覆盖行是 PIT 名称且 0 mismatch（300116/300156 与随机更名股样本 interval PASS），另有 4,044 行早于该股首个 namechange_full 区间，保留 stock_basic fallback/UNKNOWN，非 PIT，仅人工定位；industry_snapshot/list_date 当前快照（非 PIT，仅人工定位）。**sanity**：随机 20 + 大涨 5 + 大跌 5 + 先跌后涨 5 + 先涨后跌 5，ret/MFE/MAE 公式机器断言 PASS（全天高于/低于成本、日内跨越成本三态验证）。**SIGPATH-C bookkeeping**：新增 `raw_data_manifest.json`、`raw_partition_manifest.csv`、`cross_table_parity.json`、`wide_long_value_parity.json`、`path_math_invariants.json`、`manual_index_parity.json`、`pit_name_audit.json`、`missing_horizon_by_available_days.csv`、`date_boundary_audit.json`；raw parquet/CSV 不入 Git，canonical local copy 在 `results/evidence/sigpath/`，downstream study 必须先校验 parquet SHA256。**本阶段无 A/B/C/D 结论分类、不判断策略有效性、未改任何参数、未跑组合、未读 2025–2026**。canonical: `results/evidence/sigpath/README.md` + `results/evidence/sigpath/`。
...
| SIGPATH Signal Path Audit Registry | `research/sigpath/registries/` | `94bd041d21a695557df3d9596f26385b4fc8f1bf`（SIGPATH-A）→ `8bb98867988bf2b8d3d0304974391ec9276a72ba`（A2）→ `ea369277546d73957f6b5fd94700a03f21788173`（A3，SHA `4336000d…`） |
| REG1 Squeeze/ADX Regime Registry | `research/regime/registries/` | `6b7dfb5`（REG1-A，SHA `ce8460a6…`） |

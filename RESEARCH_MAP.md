...
S1 Signal Selectivity ──► BB threshold D HARMFUL / RSI C / Sector N/A (ACCEPTED)
   │
   ▼
★ CURRENT: WAITING EXTERNAL AUDIT — D1（PIT 行业归属 B + 财务披露 A，数据层已建）；外部审计通过后按 gate 进入 S3 FUNDAMENTAL DISTRESS（两者都 A/B 时优先）★
```
...
- **D1（进行中，DEVELOPMENT DIAGNOSTIC / WAITING EXTERNAL AUDIT）：** PIT CONTEXT DATA FOUNDATION — 为 2020–2024 每个 B20 signal date 建立可审计 PIT SECTOR（申万 2021 L1 + index_member in/out 重建，**信号级 coverage 94.555%**，缺失 98.7% 为申万首次纳入滞后——真实 PIT 特性；3,214 变更股可重建；spotcheck PASS → **B**）与 PIT FUNDAMENTAL（fina_indicator/income/cashflow/express/forecast 5,147 只全覆盖；**financial 100% / TTM 98.80% / forecast 94.73% / express 37.16%**；revision as-of selector PASS（57,680 修订期）；100 例 spotcheck 0 fail → **A**）；NEWS NOT_READY；红队 7 项 CLEAN；I1–I13 PASS；**DATA FOUNDATION ONLY**，禁策略测试与 outcome 读取；SECTOR A/B → S2，FUNDAMENTAL A/B → S3（都 A/B 优先 **S3 FUNDAMENTAL DISTRESS**）；2025–2026 CLOSED。registry: `research/context/registries/PIT_CONTEXT_D1_REGISTRY.csv`（SHA `6168e104…`）。

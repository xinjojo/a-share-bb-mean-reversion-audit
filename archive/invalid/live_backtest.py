...
def run_backtest(df, top_n=1, max_levels=5, level_cash=200_000, time_stop_days=None,
    ...
    # 日期切片（用于walk-forward；布林带已在prepare_data预计算）
    ...
    for i, d in enumerate(days):
        ...
        # 兜底：止盈/时间止损当日清仓后（sold_today=True）equity=现金（股票市值0）
...
if __name__ == '__main__':
    ...
    configs = [
        dict(top_n=1, max_levels=5, time_stop_days=None, label='Top1_5层'),
        dict(top_n=1, max_levels=5, time_stop_days=30, label='Top1_5层_时间止损30'),
        dict(top_n=3, max_levels=5, time_stop_days=None, label='Top3_5层'),
        dict(top_n=3, max_levels=5, time_stop_days=30, label='Top3_5层_时间止损30'),
        dict(top_n=3, max_levels=3, time_stop_days=None, label='Top3_3层'),
        dict(top_n=5, max_levels=5, time_stop_days=None, label='Top5_5层'),
        dict(top_n=5, max_levels=5, time_stop_days=30, label='Top5_5层_时间止损30'),
        dict(top_n=10, max_levels=5, time_stop_days=None, label='Top10_5层'),
        dict(top_n=10, max_levels=5, time_stop_days=30, label='Top10_5层_时间止损30'),
        dict(top_n=1, max_levels=1, time_stop_days=None, label='Top1_1层'),
        dict(top_n=3, max_levels=1, time_stop_days=None, label='Top3_1层'),
        dict(top_n=5, max_levels=1, time_stop_days=None, label='Top5_1层'),
    ]
    summary = []
    for cfg in configs:
        label = cfg.pop('label')
    ...
    sm.to_csv(os.path.join(PROJECT_ROOT, 'results', 'live_summary.csv'), index=False)
    print('\n===== 汇总 =====', flush=True)

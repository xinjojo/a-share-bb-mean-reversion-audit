...
def run_backtest(df, top_n=1, max_levels=5, level_cash=200_000, time_stop_days=None, rank_by="amount",
    ...
    for i, d in enumerate(days):
        ...
        if pos is None and not sold_today:
            ...
            if len(pool) > 0:
                ...
                if len(pool) > 0:
                    top = pool.nlargest(top_n, rank_by)
...
if __name__ == '__main__':
    ...
    configs = []
    for dim in ['amount', 'vol']:
        for n in [5, 10, 20, 30, 50]:
            configs.append(dict(top_n=n, max_levels=5, time_stop_days=None, rank_by=dim,
                                label=f'{dim}_Top{n}_5层'))
    summary = []
    for cfg in configs:
        label = cfg.pop('label')
        ...
        eq.to_parquet(os.path.join(PROJECT_ROOT, 'results', f'pool_{label}.parquet'))
        tr.to_csv(os.path.join(PROJECT_ROOT, 'results', f'pool_{label}_trades.csv'), index=False)
        print(f'  {label}: 总收益{stats["总收益%"]}%, 年化{stats["年化收益%"]}%, 回撤{stats["最大回撤%"]}%, '
              f'Sharpe{stats["Sharpe"]}, 交易{stats["交易次数"]}次, 胜率{stats["胜率%"]}%, '
              f'资金利用{stats["资金利用率%(持仓天数占比)"]}%', flush=True)

    sm = pd.DataFrame(summary)
    sm.to_csv(os.path.join(PROJECT_ROOT, 'results', 'pool_summary.csv'), index=False)
    print('\n===== 汇总 =====', flush=True)
    print(sm[['配置', '总收益%', '年化收益%', '最大回撤%', '年化波动%', 'Sharpe', '交易次数', '胜率%',
              '盈亏比', 'ProfitFactor', '资金利用率%(持仓天数占比)']].to_string(index=False), flush=True)

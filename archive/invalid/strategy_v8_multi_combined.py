...
def main():
    ...
    # 加载合并数据
    print('加载合并行情数据...', flush=True)
    t0 = time.time()
    combined = pd.read_parquet(os.path.join(project_root, 'data', 'combined_daily.parquet'))
    combined['date'] = pd.to_datetime(combined['date'])
    combined = combined.set_index(['date', 'ts_code'])
    print(f'加载完成: {len(combined):,}行, 耗时{time.time()-t0:.1f}s', flush=True)
    ...
    for i, date in enumerate(all_dates):
        # 获取当天所有股票数据
        try:
            day_data = combined.xs(date, level='date')
        except KeyError:
        ...
        for ts, pos in positions.items():
            if ts in day_data.index:
                total_position_value += pos['shares'] * day_data.loc[ts, 'close']
            else:
        ...
        # 新买入（按成交额从小到大排序，选成交额最小的）
        if len(positions) < MAX_POSITIONS and date in signal_dates:
            for ts, amount in reversed(signal_dates[date]):
    ...
    nav_df.to_csv(f'results/reports/nav_v8_multi3_mv30_minamt_{timestamp}.csv')
    trades_df.to_csv(f'results/trades/trades_v8_multi3_mv30_minamt_{timestamp}.csv', index=False)
    print(f'\n结果已保存', flush=True)

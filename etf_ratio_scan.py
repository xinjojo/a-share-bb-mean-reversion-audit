...
if __name__ == '__main__':
    ...
    configs = [
        dict(top_n=10, max_levels=5, time_stop_days=None, etf_enabled=False, etf_ratio=0.0, label='纯股票(比例0%)'),
        dict(top_n=10, max_levels=5, time_stop_days=None, etf_enabled=True, etf_ratio=0.3, label='ETF目标30%'),
        dict(top_n=10, max_levels=5, time_stop_days=None, etf_enabled=True, etf_ratio=0.5, label='ETF目标50%'),
        dict(top_n=10, max_levels=5, time_stop_days=None, etf_enabled=True, etf_ratio=0.7, label='ETF目标70%'),
        dict(top_n=10, max_levels=5, time_stop_days=None, etf_enabled=True, etf_ratio=1.0, label='ETF目标100%'),
    ]
    summary = []
    for cfg in configs:
        label = cfg.pop('label')
        ...
        eq.to_parquet(os.path.join(PROJECT_ROOT, 'results', f'ratio_{label}.parquet'))
        tr.to_csv(os.path.join(PROJECT_ROOT, 'results', f'ratio_{label}_trades.csv'), index=False)
        print(f'  {label}: 总收益{stats["总收益%"]}%, 回撤{stats["最大回撤%"]}%, ETF利用{stats["ETF利用率%"]}%', flush=True)

    sm = pd.DataFrame(summary)
    sm.to_csv(os.path.join(PROJECT_ROOT, 'results', 'etf_ratio_summary.csv'), index=False)
    print(sm[['配置', '总收益%', '年化收益%', '最大回撤%', 'Sharpe', '股票利用率%', 'ETF利用率%', '总资金利用率%']].to_string(index=False), flush=True)

#!/usr/bin/env python3
"""S2.1 Runner — Corrected Control (etf_enabled=False) + 200 random simulations.

Root cause: S2 engine used etf_enabled=True (default), but official G0 uses etf_enabled=False.
This caused idle cash to be invested in ETF 513500, adding ~+9.7pp extra return.

Fix: set etf_enabled=False, day_range=(0, N2024) to exactly match official G0.
"""
import sys, os, time
import numpy as np
import pandas as pd
...
print('='*60)
print('S2.1 CORRECTED RUNNER — etf_enabled=False, 2020-2024')
...
print(f'  Total days: {len(days)}, N2024={N2024}, last day={days[N2024-1].date()}')
...
# ===== 2. Corrected Control (etf_enabled=False) =====
print('\n[2] Running Corrected Control (etf_enabled=False, 2020-2024)...')
t0 = time.time()
eq_ctrl, tr_ctrl = run_fast_multi_strict_c_s2(
    ...
    etf_enabled=False, day_range=(0, N2024))
...
print(f'  Corrected:   TR {ctrl_metrics["total_return_pct"]}%, {ctrl_metrics["n_trades"]} trades, WR {ctrl_metrics["win_rate_pct"]}%, PF {ctrl_metrics["profit_factor"]}, MaxDD {ctrl_metrics["maxdd_pct"]}%')
print(f'  TR diff: {ctrl_metrics["total_return_pct"] - 30.295:.3f}pp')

# Save corrected control
tr_ctrl.to_csv(os.path.join(OUT, 's21_control_trades.csv'), index=False)
eq_ctrl.to_csv(os.path.join(OUT, 's21_control_equity.csv'), index=False)
pd.DataFrame([ctrl_metrics]).to_csv(os.path.join(OUT, 's21_control_summary.csv'), index=False)

# ===== 3. Corrected random simulations =====
print(f'\n[3] Running {N_SIM} corrected random simulations (etf_enabled=False)...')
...
for sim_id in range(N_SIM):
    ...
    if sim_id < 10:
        tr_r.to_csv(os.path.join(OUT, f's21_random_trades_seed{seed}.csv'), index=False)
...
results_df.to_csv(os.path.join(OUT, 's21_random_distribution.csv'), index=False)
print(f'\n  All {N_SIM} simulations complete.')

# ===== 4. Corrected percentiles =====
print('\n[4] Corrected Control percentiles...')
pct_rows = []
for metric in ['total_return_pct', 'sharpe', 'maxdd_pct', 'profit_factor', 'win_rate_pct', 'n_trades']:
    ...
    if metric == 'maxdd_pct':
        ...
        p_random_better = (rand_vals > ctrl_val).mean() * 100
    pct_rows.append({
        'metric': metric, 'control_value': ctrl_val,
...
pct_df.to_csv(os.path.join(OUT, 's21_control_percentiles.csv'), index=False)
...
pd.DataFrame([summary]).to_csv(os.path.join(OUT, 's21_random_vs_amount_summary.csv'), index=False)
print(f'\n  CORRECTED SUMMARY:')

"""ML-BB PHASE 2 ENGINE — 冻结真实账户公司行为引擎 + 注入式 ML hook（BAD veto / ADD_ON ranking）。
来源: src/strict_c_earlyexit_ca.py 逐字复制（真实账户公司行为会计冻结口径）。
ML hook 全部默认关闭: ml_veto=None, ml_add_rank=None 时行为与冻结引擎逐字一致（C0 parity 用）。
实验副本, 不修改冻结引擎。
    ...
    for i, d in enumerate(days):
        ...
        if pending_sell:
            for tc in list(pending_sell):
                ...
                pending_sell.discard(tc)
        if pending_add:
            # ---- ML hook: ADD_ON 资源竞争排序 (仅竞争时, 不取消任何合法加仓) ----
            if ml_add_rank is not None and len(pending_add) >= 2:
                _ei = etf_idx.get(d)
                _eopx = etf_open[_ei] if _ei is not None and not np.isnan(etf_open[_ei]) else np.nan
                _avail = cash + (etf_sh * _eopx * (1 - slip) if etf_enabled and not np.isnan(_eopx) else 0.0)
                if len(pending_add) * level_cash > _avail:
                    # ml_add_rank(tclist, exec_date, pdesc) — pdesc: {tc: signal_date}
                    _order = list(ml_add_rank(list(pending_add.keys()), str(d.date()), pending_add))
                    if ml_events is not None:
                        ml_events.append(dict(date=str(d.date()), kind='ADD_RANK',
                                              order=_order, n=len(_order), action='reordered'))
                    pending_add = {tc: pending_add[tc] for tc in _order if tc in pending_add}
        ...
        for pos in positions:
            ...
            if (not np.isnan(bb_lo) and dd['close_adj'][j] < bb_lo
                ...
                pending_add[pos['ts_code']] = str(d.date())
            ...
            if valid.any():
                ...
                for k in order:
                    ...
                    if (not np.isnan(dd['bb_lower'][j]) and dd['close_adj'][j] < dd['bb_lower'][j]
                        ...
                        # ---- ML hook: BAD VETO (最高风险 20% 的 NEW_ENTRY 不允许占新 K 槽) ----
                        if ml_veto is not None and ml_veto(tc, str(d.date())):
                            if ml_events is not None:
                                ml_events.append(dict(date=str(d.date()), ts_code=tc,
                                                      kind='BAD_VETO', action='vetoed'))
                            continue

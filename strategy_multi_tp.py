...
多股策略：固定止盈 + 时间止损
- 股票池：中证500（可配置）
- 选股：Top N by 成交额（可配置排序方向）
- 同时持有：MAX_POSITIONS只
- 买入：BB Lower信号，收盘价，跌停不买
- 加仓：再次跌破BB Lower，加20%，最多5层
- 卖出：固定止盈TP_RATIO + 时间止损MAX_HOLDING_TRAYS
"""
import sys, os, time
...
def main():
    ...
    MAX_POSITIONS = 3
    TOP_N = 3
    SORT_DIRECTION = 'desc'  # 'asc'=从小到大, 'desc'=从大到小
    USE_INDEX_POOL = True
    INDEX_CODE = '000905.SH'

    # 卖出参数（固定止盈+时间止损）
    TAKE_PROFIT_RATIO = 0.02   # 止盈2%
    MAX_HOLDING_TRAYS = 20      # 最多持有20个交易日
    ...
    print(f"卖出：止盈{TAKE_PROFIT_RATIO:.0%} + 时间止损{MAX_HOLDING_TRAYS}个交易日")
    ...
    # ===== 回测主循环 =====
    cash = INITIAL_CASH
    positions = {}  # {ts_code: {shares, avg_cost, level, entry_day_idx, highest_high}}
    trades = []
    nav = []
    ...
    for i, d in enumerate(trade_days):
        ...
        for ts, pos in list(positions.items()):
            ...
            if holding_trays >= 1:  # T+1，至少持有1个交易日后才能卖
                # 1. 止盈：盘中最高价 >= 平均成本 * (1 + TP)
                tp_price = pos['avg_cost'] * (1 + TAKE_PROFIT_RATIO)
                if row['high'] >= tp_price:
                    sell = True
                    sell_price = tp_price  # 日线近似，假设以止盈价成交
                    reason = 'TAKE_PROFIT'

                # 2. 时间止损：持有交易日 >= MAX_HOLDING_TRAYS
    ...
    if n_sells > 0:
        ...
        for reason in sorted(sell_trades['reason'].unique()):
            ...
            print(f"  {reason:<20}: {cnt:>4}笔 ({cnt/n_sells*100:>5.1f}%), 胜率{wr:>3.0f}%, 平均盈亏{avg_pnl:>8,.0f}, 平均持仓{avg_ht:>5.1f}天")
    ...
    tag = f"tp2_ts20_{pool_tag}_{sort_tag}_top{TOP_N}_p{MAX_POSITIONS}"

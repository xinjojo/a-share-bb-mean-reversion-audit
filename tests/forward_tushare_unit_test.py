...
def t11b():
    ...
    synth.to_parquet(os.path.join(inc, 'dividend_20260915.parquet'), index=False)
    ...
    os.remove(os.path.join(inc, 'dividend_20260915.parquet'))
...
def t11c():
    ...
    inc = p.INC
    os.makedirs(inc, exist_ok=True)
    synth = pd.DataFrame(dict(ts_code=['000001.SZ', '600000.SH'], trade_date='2026-09-07',
    ...
    synth.to_parquet(os.path.join(inc, 'daily_20260907.parquet'), index=False)
    adj = pd.DataFrame(dict(ts_code=['000001.SZ', '600000.SH'], date='2026-09-07', adj_factor=[1.1, 2.2]))
    adj.to_parquet(os.path.join(inc, 'adj_20260907.parquet'), index=False)
    try:
        ...
        os.remove(os.path.join(inc, 'daily_20260907.parquet'))
        os.remove(os.path.join(inc, 'adj_20260907.parquet'))
...
def t11d():
    ...
    try:
        ...
        os.remove(os.path.join(inc, 'namechange_forward.parquet'))
...
def t11e():
    ...
    # 清理测试行（保留真实告警）
    al = pd.read_csv(u.ALERT_PATH, dtype=str)
    al = al[~((al['ts_code'] == '000001.SZ') & (al['date'] == '2026-08-26') & (al['source'] == 't'))]
    if len(al):
        al.to_csv(u.ALERT_PATH, index=False)
    else:
        os.remove(u.ALERT_PATH)
